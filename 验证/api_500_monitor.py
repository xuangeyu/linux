...
def check_time():
    ...
    past_time = (datetime.datetime.now() - datetime.timedelta(hours=8) - datetime.timedelta(minutes=5)).strftime('%Y-%m-%dT%H:%M:%S')
    ...
    complex=zero + datetime.timedelta(minutes=5)
    if now < complex:
        ret=complex-now
